#!/usr/bin/env python3
"""Generate an English daily economy and markets news brief."""
...
def fetch_text(url: str) -> str:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "EconomyBrief/1.0 (+https://github.com/rajeshphy/economic-news)",
...
def default_source_weight(source_name: str, section: str) -> int:
    ...
    if any(source in name for source in ("rbi", "sebi", "nse", "bse")):
        return 5
    if any(source in name for source in ("markets", "macro", "global market", "crude")):
        return 4
    if any(source in name for source in ("earnings", "economy", "corporate")):
        return 3
    return 1
...
def section_keywords(section: str, settings: dict) -> list[str]:
    ...
    default_world = (
        "federal reserve,fed,us markets,nasdaq,dow,treasury yields,crude oil,brent,global markets,dollar index"
    )
...
def keyword_set(text: str) -> set[str]:
    ...
    stopwords = {
        ...
        "news", "latest", "today", "google", "india", "indian", "world",
        "market", "markets", "economy", "economic", "says", "said", "could", "first",
...
def score_story_group(group: list[NewsItem], settings: dict) -> tuple[int, list[str]]:
    ...
    public_hits = keyword_hits(
        ...
        configured_keywords(
            ...
            "research_keywords",
            (
                "rbi,sebi,nse,bse,nifty,sensex,rupee,inflation,gdp,iip,pmi,"
                "monetary policy,repo rate,bond yield,crude oil,brent,earnings,"
                "profit,revenue,credit growth,liquidity,fii,dii,global cues,us markets,fed"
            ),
        ),
    )
    if public_hits:
    ...
    classroom_hits = keyword_hits(
        ...
        configured_keywords(
            settings,
            "classroom_keywords",
            "inflation,gdp,interest rate,repo rate,rupee,crude oil,bond yield,earnings,valuation,liquidity",
        ),
    )
    if classroom_hits:
...
- The SUMMARY must not copy the first bullet. It should combine 2 to 4 themes, for example: "RBI policy, inflation, earnings, crude oil, and global market cues."
- Keep SUMMARY under 160 characters.
- Then produce exactly two sections:
  SECTION: India Markets
  SECTION: Global Cues
...
- When both sections have strong candidates, include at least one India item and one global-cue item. Do not force a weak item.
...
- Prioritize items useful for market awareness: RBI, inflation, GDP, rupee, crude oil, bond yields, SEBI/NSE/BSE rules, earnings, sector policies, FII/DII flows, and global cues affecting Indian markets.
- Do not give buy/sell/hold recommendations, target prices, or personalized investment advice.
- Explain why each point may matter for markets or investor watchlists.
...
- Do not merge unrelated earnings, policy decisions, macro data, regulatory updates, or global cues just because they share a broad theme.
...
- Prefer market-moving facts over stock tips, hype, sponsored brokerage calls, penny-stock stories, or routine press releases.
- End each bullet with source ids using this exact format: Sources: [I1], [I3] or Sources: [W2]
- Do not include inline URLs.
- Format bullets as: - **Short topic:** one concise synthesized sentence explaining what happened and why it matters for markets. Sources: [I1], [W3]
...
def split_digest_header(summary: str) -> tuple[str, str, str]:
    ...
    return title or "Economic News Brief", teaser, "\n".join(remaining).strip()
...
def clean_title(value: str) -> str:
    ...
    return title[:80].rstrip(" ,;:") if title else "Economy Brief"
...
def generic_title(title: str) -> bool:
    ...
    generic_titles = {
        "economic brief",
        "economic news brief",
        "daily economic news brief",
        "india and world economic brief",
    }
...
def build_post(summary: str, items: list[NewsItem], used_ai: bool, points_per_section: int) -> Path:
    ...
    post_path = POSTS / f"{now_ist.date().isoformat()}-economy-brief.md"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the daily economic news brief.")
    ...
    api_key = os.environ.get("ECONOMIC_API_KEY")

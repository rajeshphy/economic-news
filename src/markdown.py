...
try:
    ...
    from .filter import keyword_set, normalize_match_text
except ImportError:
    from common import IST, POSTS, SITE_TITLE, NewsItem, clean_text
    from filter import keyword_set, normalize_match_text
...
def source_relevance_score(text: str, item: NewsItem) -> int:
    ...
    phrase_pairs = (
        ("reserve bank of india", "reserve bank of india"), ("rbi", "rbi"),
        ("securities and exchange board of india", "securities and exchange board of india"), ("sebi", "sebi"),
        ("nifty", "nifty"), ("sensex", "sensex"), ("inflation", "inflation"), ("gdp", "gdp"),
        ("rupee", "rupee"), ("crude oil", "crude oil"), ("brent", "brent"),
        ("earnings", "earnings"), ("profit", "profit"), ("federal reserve", "federal reserve"),
        ("bond yield", "yield"), ("fii", "fii"), ("dii", "dii"), ("monetary policy", "monetary policy"),
    )
...
def sources_to_html(items: list[NewsItem]) -> str:
    ...
    for item in items:
        ...
        lines.append(f'  <li><a href="{url}" target="_blank" rel="noopener noreferrer">[{item.item_id}] {title}</a> {source}</li>')
    lines.append("</ul>")
    return "\n".join(lines)
